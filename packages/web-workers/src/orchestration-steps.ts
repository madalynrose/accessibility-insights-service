...
export interface OrchestrationSteps {
    ...
    invokeSubmitScanRequestRestApi(url: string, scanNotificationUrl?: string): Generator<Task, string, SerializableResponse>;
...
export class OrchestrationStepsImpl implements OrchestrationSteps {
    ...
    public *waitForScanCompletionNotification(scanId: string): Generator<Task, ScanCompletedNotification, SerializableResponse & void> {
        ...
        const waitEndTime = waitStartTime.clone().add(900, 'seconds');
    ...
    public *invokeSubmitScanRequestRestApi(
        ...
        const requestData: CreateScanRequestData = {
            scanUrl: url,
            priority: 1000,
            notifyScanUrl: scanNotificationUrl,
        };
        ...
        this.logOrchestrationStep(`Orchestrator submitted scan with scan Id: ${scanId} and url ${scanNotificationUrl}`);
